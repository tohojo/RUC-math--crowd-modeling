...
agents ability to maneuver around in the environment.

The agents do not have path finding, which 
mean that they can not escape a more complex environment with a lot of rooms 
and corridors. An improvement of the model would then be to implement 
path finding for the agents, so that they can manoeuvre around in other more 
complex cases then the ones we have. A example of a social force model with 
pathfinding is the HiDAC model \cite{HiDAC}.

In other models such as the HiDAC, agents can share information about the 
environment, such that the agents have a better idea of where the exit is/are \cite{HiDAC}. 
In a real panic situation you would expect people to communicate when evacuation 
a building.
...
from the walls.
%this part should discuss some dynamica that the agents lack, such as falling
