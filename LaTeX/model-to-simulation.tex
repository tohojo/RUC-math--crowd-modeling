...
\begin{itemize}
    ...
    \item \textbf{Relaxation time:} The relaxation time, $\tau$, is set to $1$ 
        second in \cite{self-org}, and $0,5$ second in \cite{social-force}. 
\end{itemize}
...
\begin{figure}[h]
    ...
    \begin{tikzpicture}[scale=1.2]
		\node (p) at (-0.2,1.5) [draw,circle,inner sep=2.5pt] {};
		\node (el) at (-0.5,2) [fill,circle,inner sep=1pt] {};
		\node (er) at (0.5,2) [fill,circle,inner sep=1pt] {};

		\draw (p) node [below,xshift=5pt] {$\alpha$};
        \draw [thick] (0.5,2) -- (2,2) -- (2,-1) -- (-2,-1) -- (-2,2) --(-0.5,2);
        \draw [dashed,opacity=0.4] (0.5,2.5) -- (0.5,-1)
        (-0.5,2.5) -- (-0.5,-1);
    	\draw [dotted,thin] (p) -- (el);
		\draw [dotted,thin] (p) -- (er);
		\draw [very thin, -latex] (p) -- (0,2.5);
    \end{tikzpicture}

    \caption[Preventing repulsion from doorways]{Preventing repulsion from 
    doorways. Pedestrians between the dashed lines do not get repulsed from 
    the marked endpoints unless the distance (indicated by the dotted lines) 
    is smaller than the pedestrian's radius.}
    \label{fig:doorways}
\end{figure}
