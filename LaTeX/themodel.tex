...
\begin{figure}[ht]
    ...
    \subfloat[Notation of pedestrian properties.]{
        \includegraphics[height=6cm]{Figures/NotationOfPedestrian.pdf}
        \label{subfig:notation}
    }
    \subfloat[Main forces acting on a pedestrian.]{
        \includegraphics[height=6cm]{Figures/ForceModel.pdf}
        \label{subfig:forces}
    }
...
\begin{figure}[ht]
    \centering
    \begin{tikzpicture}
        \draw [o-o,thick]  (-3,1) -- (3,1);
        \node (pg) at (0.5,1) [draw,circle,inner sep=3pt,fill=red!80!black] {};
        \node at (pg) [anchor=south west] {$\overrightarrow{p_{\gamma \alpha}}$};

        \draw (0.5,-0.5) circle[radius=0.5];
        \draw [|-|,dashed] (0.5,1) -- (0.5,-0.5);

        \node at (1.2,0.4) {$d_{\alpha \gamma}(t)$};

        \node at (1.0,-1.0) [font=\Large] {$\alpha$};
        \node at (-2.5,1.3) [font=\Large] {$\gamma$};
    \end{tikzpicture}
