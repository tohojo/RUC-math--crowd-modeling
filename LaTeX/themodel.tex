...
of pedestrian $V_{\alpha}^{\text{max}}=C V_{\alpha}^{Id}$, where C is some constant larger than 1. The maximum desired speed is given as a parameter and
is calculated as $$. This speed is the maximum speed the pedestrian will
...
desired speed because; $\langle V_\alpha(t)\rangle \rightarrow V^{Id}_\alpha$ implies  $\eta_\alpha(t)\rightarrow0$ which implies $V^d_\alpha(t))\rightarrow $V^{Id}_\alpha$$

Similar we see that $\langle V_\alpha(t)\rangle \rightarrow 0$ implies $\eta_\alpha(t)\rightarrow0$ which implies $V^d_\alpha(t))\rightarrow V^{max}_\alpha$

Since $V^{\text{max}}_\alpha$ is set to be larger than $V^{Id}_\alpha$, this
means that as the average speed goes up, the desired speed goes down.  If
the desired speed is in some case is lower than the actual speed, this will result in a
negative desired movement force due to~\eqref{eqn:desired-force}, and vice
versa. So you could say that the desired movement force acts as a
stabilising force, that accelerates the pedestrian when it is moving slower
than its desired speed, and decelerate it when it is moving faster, and
...
The \emph{interaction force} between pedestrians is a summation of all the forces between a pedestrian $\alpha$ and the other pedestrians denoted $\beta$. 
The interaction force is repulsive in nature, and should, if high enough, prevent the pedestrians from overlapping or
...
\beta}$ is the distance between the centres of the pedestrians, as seen in figure \ref{subfig:notation-interaction}.
...
Where $0\leq \lambda \leq 1$. This and the fact that $-1\leq cos(\theta_{\alpha \beta})\leq 1$ 
implies $\lambda \leq w \leq 1$. So $w$ is modifies the force given by
...
arise because the projection it not always a point on the wall; this is discussed further
