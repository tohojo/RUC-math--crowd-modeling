...
social force model is an pedestrian based model, this means that we describe the 
...
motion but rather a measure of the pedestrians motivation for acting in specific 
...
\begin{figure}[ht]
    ...
    of forces acting on an pedestrian]{Illustration of the forces acting on an 
    pedestrian $\alpha $. $ \beta $ is another pedestrian and the grey bar on the top 
    represents a wall. $ \overrightarrow{f_{\alpha\beta}} $ is the repulsive force from 
    pedestrian $ \beta $, and $ \overrightarrow{f_{\alpha B}} $ is the repulsive force from 
    the wall. $ \overrightarrow{f^{0}_{\alpha}} $ is a force that represents pedestrian $ 
    \alpha $'s desire to reach the exit.  The x and y axes are defined by the 
    Cartesian coordinate system.}
    \label{ForceModel}
\end{figure}

\subsection{The equation of motion for pedestrian $ \alpha $:}
The purpose of the model is to describe how each pedestrian moves so an equation of 
motion for each pedestrian is needed. They way to get this equation of motion is 
much like one would in physics by summing up all the forces acting on the 
pedestrian. This gives rise to an acceleration of the pedestrian and from this one is 
able to derive the exact movement of the pedestrian. Again it is important that 
this is a method borrowed from physics, we are not working with real physical 
forces here.

Each induvidual pedestrian has a series of parameters describing the features of 
the pedestrian. An pedestrian is represented by a circle with radius $R_{\alpha}$. The 
pedestrian has a position vector $\overrightarrow{p_{\alpha}}$ and a velocity vector $\overrightarrow{V_{\alpha}}$ at 
all times t. Furthermore each pedestrian has a desired direction $\overrightarrow{e_{\alpha}}$ 
in which it would like to move. An illustration of the notation can be seen in 
figure \ref{fig:NotationOfPedestrian}.

\begin{figure}[hb]
    \centering
    {\includegraphics[scale=0.35]{Figures/NotationOfPedestrian.pdf}} 
    \caption[Notation of an pedestrian]{Illustration of the visual presentation of 
    the mathematical notations for position and velocity. As for pedestrian $ 
    ...
    coordinate system.}
    \label{fig:NotationOfPedestrian}
\end{figure}

The change in position $ \overrightarrow{p_{\alpha}} $ per time of pedestrian $\alpha$ is the 
...
The change of velocity per time is the acceleration of pedestrian $\alpha$, which 
is due to all the forces acting on the pedestrian is given by:
...
is a repulsive force from other pedestrians and $\overrightarrow{f_{\alpha i}}$ is a 
attractive force.
...
\subsection{The attractive forces between some pedestrians}
...
sights or familiar persons the pedestrian prefer to be close to, such as friends 
and family. The mathematical structure of this force is the same as the force 
from other pedestrians, however it is opposite in algebraic sign and has different 
constants. 
