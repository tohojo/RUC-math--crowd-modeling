...
in great detail. As mentioned earlier in section~\ref{subsec:ABapproach} the 
...
agent has a position vector $\vec{r_{\alpha}}$ and a velocity vector $\vec{V_{\alpha}}$ at 
...
role in the model. But before we do this we would like to comment on the 
notation of the mathematics in the model. Pay close attention to the vectors, 
there are some parameters with the same symbols but that are not the same and it 
is easy to get confused. For example $r_{\alpha}$ is the radius of agent $\alpha$ 
but $\vec{r_{\alpha}}$ is the position vector of agent $\alpha$. In general, 
...
however not the case in the main article we are working with.In another articles [ ] by 
the same author the repulsive potential from the wall is given by: %TODO: Find this reference!
...
is trying to avoid the wall as it is expected from real life situations. [Helbing and Molnár, 1995]. %real references, please.
...
$w\left(\phi_{\alpha \beta}\right)$ is a function of the angle between two pedestrians. It is given as: 
