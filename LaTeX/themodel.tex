...
\begin{figure}[hb]
    ...
    \caption[Notation of an agent]{\small{Illustration of the visual presentation of the mathematical notations for position and velocity. As for agent $ \alpha $,
...
\caption[Illustration of agent $\alpha$'s average speed]{\small{Illustration of agent $\alpha$'s average speed in the desired direction, i.e. toward the exit.
$\vec{r_{\alpha}}$ is the displacement vector of $\alpha$, while $\vec{r^{E}_{\alpha}}$ is $\alpha$'s average velocity toward the exit.}}
\label{impatience}
\end{figure}
...
\caption[The impatience factor]{\small{Illustration on the correlation between the function about the desired speed $ V_{\alpha}^{0}(t) $ 
and the average speed in the desired direction of motion $ \overline{V}_{\alpha} \left( t \right) $, and the graph of
$ V_{\alpha}^{0}(t) = \left[ 1 - \frac{V_{\alpha}^{max}}{V_{\alpha}^{0}(0)} \right]\overline{V}_{\alpha} \left( t \right) + V_{\alpha}^{max} $
intersects with the axis at $ \left( 0 , V_{\alpha}^{max} \right)  $ and $ \left(V_{\alpha}^{max} \frac{V_{\alpha}^{0} \left( 0 \right) }{V_{\alpha}^{max}-V_{\alpha}^{0} \left(0 \right)} , 0 \right)  $.  When $\alpha$'s velocity is at maximum the impatience gets low, and vice versa.}}
...
$\vec{V_{\alpha}}$ is the velocity vector for $\alpha$.}}
...
\begin{figure}[ht]
    ...
    \caption[Notation of the interaction between two agents]{\small{Illustration of the notation for the interaction between agents.
...
Here $A_{\alpha}^{1}$, $A_{\alpha}^{2}$, $B_{\alpha}^{1}$ and $B_{\alpha}^{2}$ 
are all constants that can differ for each agent.
 $\lambda_{\alpha}$  is chosen to be $\approx 0$, to take into account that agents
responts more to what is happening in front of the agent rather than what is happening
behind the agent \cite{self-org}.
...
\begin{figure}
    ...
    \caption[Psysical interaction]{\small{Illustration of the function about the interaction force 
...
\begin{figure}[hb] %with some more comments i think that this figure could serve as a summation of the entire section
    ...
    \caption[Overview of the model]{\small{Illustration of an overview of how the model is put together. The different equations and their notation is written to give the 
