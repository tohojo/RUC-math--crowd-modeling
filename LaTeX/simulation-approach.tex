...
in code listing~\ref{lst:pedestrian-repulsion}. The \texttt{add\_repulsion} function 
...
% TODO: Change this code to match the changed way of doing stuff.
...
\begin{lstlisting}[caption={Code updating the pedestrian 
...
% TODO: Is it better to explain the why along with the how? Also, should we 
% elaborate on expected results, in general, when we present the measured 
% features?
...
\begin{itemize}
    ...
    \item \textbf{Flow rate:} Flow rate is measured along a line segment 
        ...
        be perpendicular to the primary movement direction of the pedestrians. The 
        simulation keeps track of at what time each pedestrian first comes into 
        ...
        when the pedestrians cross the line. The average flow rate is then computed 
        as the number of pedestrians who have crossed the line divided by the time. 
