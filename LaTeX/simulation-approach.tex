...
\begin{itemize}
    ...
    \item \textbf{Flow rate} We want to measure the flow rate of leaving pedestrians at the exit of a room as agents
	    per second. We want to measure the maximum flow rate of the simulation and the average flow rate.
	    Part of the model geometry was made as a line seqment as the exit, where we will measure the flow rate.
	    When the simulation starts there will possibly be no flow rate since the agents first have to move to the exit
	    before they can run out, hence the lowest flow rate, 0 agents per second, would not give us any useful information,
	    where as the highest flow rate tells us how many can exit the room per second.

    \item \textbf{Density} As part of the model geometry we made a square in front of the exit, with the size $1 m^2$,
	    where we meaure the density as agent in that square. We want to measure the highest density of all timesteps
	    in a simulation, since this gives the most information of how dense the crowd was when exiting the room.
	    For saving calculations we only take the timestep with the highest density.
