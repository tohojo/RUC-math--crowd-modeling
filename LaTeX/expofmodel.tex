...
the change in position of agent $\alpha$ is given by
...
here $\vec{\xi_{\alpha}} \left( t \right)$ is a random fluctuation of agent 
$\alpha$. $\vec{f_{\alpha}} \left( t \right)$ is a summation of all the forces 
action in agent $\alpha$ from the environment and other agents. More 
...
\begin{equation}
    \vec{V_{\alpha}}^{0} = \left[ 1 - \eta_{\alpha} \left( t \right) \right] 
