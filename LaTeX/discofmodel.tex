...
\caption{\small{The function about the desired velocity $ V_{\alpha}^{0}(t) $ and the average velocity in the desired direction of motion $ \overline{V}_{\alpha} \left( t \right) $}\label{impatience}}
\end{figure}


\subsection{The repulsive interaction force}
From the given formula for calculating the repulsive force between agents in the description of the model, the part calculating the force to keep the personal space can be omitted when the agents are rather close to each other, then the calculation can be reduced as Equation (\ref{re}).
...
Taking the norms of both sides of Equation (\ref{re}), we can draw the relation between the value of $ \overrightarrow{f_{\alpha\beta}}(t) $ and $ d_{\alpha}\beta} $, as in Figure (\ref{physicalinteraction}).\\

\begin{figure}
\centering
{\includegraphics[scale=0.45]{physicalinteraction.pdf}} 
\caption{\small{The function about the interaction force $ \overrightarrow{f_{\alpha\beta}}(t) $ and the distance between two agents$ d_{\alpha}\beta} $ }\label{physicalinteraction}}
\end{figure}
